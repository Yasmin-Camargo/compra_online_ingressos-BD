    ...
    <main>
        <article class="login-container">
            ...
            if (isset($_SESSION['cnpj_login'])) {
                ...
                evento.nomecategoriaevento, categoriaevento.descricaocategoria,
                ...
                if ($retorno->rowCount() > 0) {
                    while ($row = $retorno->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<p><strong>Endereco:</strong> Rua " . $rua_endereco . ", " . $numero_endereco . ", Cidade: " . $cidade_endereco . ", CEP: " . $cep_endereco . "</p><br>";
                         ?>
                        <button class="botaoEditar" onclick="editInfoOrganizador(<?php echo $eventoID; ?>)">Editar evento</button>
                        <div id="editar-info-<?php echo $eventoID; ?>" class="login-container" style="display: none;">
                            <h1>Editar Informações</h1>
                            <form action="atualizar_evento.php" method="post">
                                <?php
                                    echo "<input type='hidden' name='idevento' value='$eventoID'>";
                                    echo "<h2>Evento $eventoCount</h2>";
                                    
                                    echo '<label for="nome">Nome:</label><br>';
                                    echo '<input type="text" id="titulo" name="titulo" value="' . $titulo_evento . '"><br><br>';
                                   
                                    echo '<label for="datahora">Data e Hora do Evento:</label><br>';
                                    echo '<input type="datetime-local" id="datahoraevento" name="datahoraevento" value="' . $dataHora_evento . '" required><br><br>';

                                    echo '<label for="descricao">Descrição do Evento:</label><br>';
                                    echo '<input type="text" id="descricao" name="descricao" value="' . $descricao_evento . '"><br><br>';

                                    echo '<label for="duracao">Duração (horas):</label><br>';
                                    ...
                                    echo '<input type="text" id="classificacao" name="classificacao" value="' . $classificacao_evento . '" required><br><br>';

                                    echo '<label for="website">Website:</label><br>';
                                    echo '<input type="text" id="website" name="website" value="' . $webSite_evento . '" required><br><br>';

                                    echo '<label for="precobase">Preço base:</label><br>';
                                    echo '<input type="text" id="precobase" name="precobase" value="' . $precobase_evento . '" required><br><br>';

                                    echo '<label for="nomecategoriaevento">Categoria:</label><br>';
                                    echo '<input type="text" id="nomecategoriaevento" name="nomecategoriaevento" value="' . $categoria_evento . '" required><br><br>';

                                    echo '<label for="descricaocategoria">Descrição da categoria:</label><br>';
                                    echo '<input type="text" id="descricaocategoria" name="descricaocategoria" value="' . $descricao_categoria_evento . '" required><br><br>';

                                    echo '<label for="nomelocal">Local:</label><br>';
                                    echo '<input type="text" id="nomelocal" name="nomelocal" value="' . $nomelocal_evento . '" required><br><br>';

                                    echo '<label for="detalhesdeacesso">Detalhes de acesso:</label><br>';
                                    echo '<input type="text" id="detalhesdeacesso" name="detalhesdeacesso" value="' . $detalhesAcesso . '" required><br><br>';

                                    echo '<label for="capacidade">Capacidade do local:</label><br>';
                                    echo '<input type="text" id="capacidade" name="capacidade" value="' . $capacidadeLocal . '" required><br><br>';

                                    echo '<label for="rua">Rua:</label><br>';
                                    echo '<input type="text" id="rua" name="rua" value="' . $rua_endereco . '" required><br><br>';

                                    echo '<label for="numero">Número:</label><br>';
                                    echo '<input type="text" id="numero" name="numero" value="' . $numero_endereco . '" required><br><br>';

                                    echo '<label for="cidade">Cidade:</label><br>';
                                    echo '<input type="text" id="cidade" name="cidade" value="' . $cidade_endereco . '" required><br><br>';

                                    echo '<label for="cep">CEP:</label><br>';
                                    echo '<input type="text" id="cep" name="cep" value="' . $cep_endereco . '" required><br><br>';

                                    echo '<label for="cnpj">Confirme o seu CNPJ:</label><br>';
